    ...
    {        
                ...
                while (cadena != null)
                {
                    ...
                    if (campos[0].Trim().Equals(busquedaDNI))
                    {
                        if (encontrado == false)
                        {
                            ...
                            cuenta.Monto = decimal.Parse(campos[4].Trim(),culture);
        ...
        static public void ModificarArchivo(List<string> lista, int posicion, Cliente cliente)
        {
            ...
            string nuevaLinea = (cliente.DNI + "," + cliente.Nombre + "," + cliente.Apellido + "," +
                cliente.Cuentas[posicion].NroCuenta + "," + cliente.Cuentas[posicion].Monto.ToString(culture) + "," +
                cliente.Cuentas[posicion].Moneda.ToString());        
            for (int i = 0; i < lista.Count - 1; i++)
            {
                for (int j = 0; j < cliente.Cuentas.Count; j++)
                {
